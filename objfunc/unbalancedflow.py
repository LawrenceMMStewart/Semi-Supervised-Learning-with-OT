"""
File: unbalancedflow.py
Description: Gradient flow between a measure consisting of 3 gaussians
on a triangle, with one gaussian consisting of more points (unbalanced)
...
from utils.graphing import scatter3D
np.random.seed(12)
...
n2 = 350
n3= 800
assert n1+n2+n3 ==n_samples , "Please respect number of points in gaussian"
#put gaussians in a triangle
d = 3
h = np.sqrt(3*d**2/4)

µ1 = [ 0. ,0. ,0.] #mean of gaussian 1 
µ2 = [ 0.,d ,0] # mean of gaussian 2 
µ3 = [0.,d/2,h]
cov = 0.1*np.eye(3)     # covariance of gaussians


g1 = np.random.multivariate_normal(µ1,cov,n1)
g2 = np.random.multivariate_normal(µ2 ,cov,n2)
g3 = np.random.multivariate_normal(µ3 ,cov,n3)

name = "unbalanced-"+str(args.batch_size)+'-'+str(args.epsilon)+'-'+str(args.exponent)
...
with open("./variables/unbalanced/"+name+".pickle" ,'wb') as f:
