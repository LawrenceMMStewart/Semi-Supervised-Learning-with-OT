...
#mids = positions of points that have missing values
#cids = positions of points that are complete
...
X = X_start.copy()
epochs = 10000
name = str(epochs)+"_"+str(batch_size)
...
for t in tqdm(range(epochs),desc = "Epoch"):
	...
	loss_fun = lambda: sinkhorn_sq_batch(Xkl,p=2,niter=10,div=True,epsilon=0.01)
...
axes[0].legend()
axes[0].set_title("Initialisation of Missing Data")
...
with open("./variables/toy_examples"+"_"+name+".pickle" ,'wb') as f:
